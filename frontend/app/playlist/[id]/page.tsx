...
export default function PlaylistDetailPage() {
    ...
    const { playTracks, addToQueue, currentTrack, isPlaying, pause, resume, queue } = useAudio();
    ...
    const handleToggleHide = async () => {
        ...
        try {
            if (playlist.isHidden) {
                ...
                toast.success("Playlist is now visible");
            } else {
                await api.hidePlaylist(playlistId);
                toast.success("Playlist hidden from your library");
            }
    ...
    const playlistTrackIds = useMemo(() => {
        return new Set(playlist?.items?.map((item: PlaylistItem) => item.track.id) || []);
    }, [playlist?.items]);

    const isThisPlaylistPlaying = useMemo(() => {
        if (!isPlaying || !currentTrack || !playlist?.items?.length) return false;
                    ...
                    <div className="flex flex-col md:flex-row items-center md:items-end gap-4 md:gap-6 w-full text-center md:text-left">
                        ...
                        <div className="flex-1 pb-2 space-y-2 md:space-y-4 max-w-full md:max-w-none">
                            <div className="flex items-center gap-2">
                                <span className="text-sm md:text-base font-bold text-white/90 drop-shadow-lg">
                                    Playlist
                                </span>
                                {isShared && (
                                    <span className="px-2 py-0.5 bg-purple-500/80 backdrop-blur-sm rounded-full text-xs font-medium text-white flex items-center gap-1">
                            ...
                            <div className="flex flex-wrap items-center gap-2 text-sm md:text-base text-white/90 font-medium">
                                ...
                                {isShared && playlist.user?.username && (
                                    <>
                                        ...
                                        <span className="drop-shadow-lg text-purple-300">
            ...
            <div className="sticky top-0 z-30 bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-white/5">
                <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 md:py-6">
                    <div className="flex items-center justify-between gap-3 md:gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            {/* Hide/Unhide Button - Available for all playlists */}
                            <button
                                onClick={handleToggleHide}
                                disabled={isHiding}
                                className={cn(
                                    "p-3 rounded-full transition-all backdrop-blur-sm border border-white/5",
                                    playlist.isHidden
                                        ? "bg-green-500/20 hover:bg-green-500/30 text-green-400 hover:text-green-300 hover:border-green-500/30"
