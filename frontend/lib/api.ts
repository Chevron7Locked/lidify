...
const getApiBaseUrl = () => {
    ...
    if (frontendPort === "3030" || frontendPort === "443" || frontendPort === "80") {
...
class ApiClient {
    ...
    // Library
    async getArtists(params?: { limit?: number; offset?: number; filter?: 'owned' | 'discovery' | 'all' }) {
        return this.request<{
            artists: any[];
            total: number;
            offset: number;
    ...
    async testSlskd(url: string) {
        return this.request<any>("/system-settings/test-slskd", {
            method: "POST",
    ...
    async getPodcastEpisodeCacheStatus(
        podcastId: string,
        episodeId: string
    ): Promise<{ cached: boolean; downloading: boolean; downloadProgress: number | null }> {
        return this.request<{ cached: boolean; downloading: boolean; downloadProgress: number | null }>(
