...
const navigation = [
    ...
    { name: "Audiobooks", href: "/audiobooks", icon: Book },
    { name: "Podcasts", href: "/podcasts", icon: Mic2 },
    { name: "Discovery", href: "/discover", icon: Sparkles },
];
...
export function Sidebar() {
    ...
    const { currentTrack, currentAudiobook, currentPodcast, playbackType } = useAudio();
    ...
    const sidebarContent = (
        <>
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            {!currentTrack && !currentAudiobook && !currentPodcast ? (
                                ...
                                <div className="text-xs text-gray-400 truncate">
                                    <span className="text-gray-500">Listening to: </span>
            ...
            {/* Navigation */}
            <nav className={cn("pt-6 space-y-1", isMobileOrTablet ? "px-6" : "px-3")}>
                {navigation.map((item) => {
            ...
            <div className="flex-1 overflow-hidden flex flex-col mt-8">
                <div className={cn("mb-4 flex items-center justify-between group", isMobileOrTablet ? "px-6" : "px-4")}>
                    <Link
                        href="/playlists"
                        prefetch={false}
                ...
                <div className={cn("flex-1 overflow-y-auto space-y-1 scrollbar-thin scrollbar-thumb-[#1c1c1c] scrollbar-track-transparent", isMobileOrTablet ? "px-6" : "px-3")}>
                    {isLoadingPlaylists ? (
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        {isShared && (
                                            <span className="shrink-0 w-1.5 h-1.5 rounded-full bg-purple-500" title={`Shared by ${playlist.user?.username || 'someone'}`} />
                                        )}
                                    </div>
                                    <div
                                        className={cn(
                                    ...
                                    >
                                        {isShared ? `by ${playlist.user?.username || 'Shared'}` : 'Playlist'} • {playlist.trackCount} track
    ...
    return (
        <>
            ...
            {/* Desktop Sidebar */}
            {!isMobileOrTablet && (
                <aside className="w-72 bg-[#0f0f0f] rounded-lg flex flex-col overflow-hidden relative border border-white/[0.03]">
                    {/* Subtle gradient overlay with noise texture effect */}
                    <div className="absolute inset-0 bg-gradient-to-b from-purple-500/[0.05] via-transparent via-50% to-yellow-500/[0.03] pointer-events-none" />
