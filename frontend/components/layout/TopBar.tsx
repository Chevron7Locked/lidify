...
import { Home, Search, Settings, RefreshCw, Power, Menu } from "lucide-react";
...
export function TopBar() {
    ...
    const { jobStatus, isPolling } = useJobStatus(scanJobId, "scan", {
        onComplete: (result) => {
            toast.success(
                `Library scan complete! Added ${result.tracksAdded} tracks, updated ${result.tracksUpdated}, removed ${result.tracksRemoved}`
            );
            setScanJobId(null);
        },
        onError: (error) => {
    ...
    const handleSync = async () => {
        ...
        // Prevent spam clicking - cooldown of 5 seconds
        const now = Date.now();
        const timeSinceLastScan = now - lastScanTime;
        if (timeSinceLastScan < 5000) {
            toast.error(
                `Please wait ${Math.ceil(
                    (5000 - timeSinceLastScan) / 1000
                )} more seconds before scanning again`
            );
            return;
        }
        ...
        try {
            ...
            toast.success(
                "Library scan started! Your music will appear shortly."
            );
        } catch (error) {
            ...
            {isMobile ? (
                <>
                    ...
                    <button
                        ...
                        className="w-12 h-12 flex items-center justify-center bg-[#0f0f0f] border border-[#262626] rounded-md text-white hover:bg-[#141414] transition-colors mr-2"
                    ...
                    <div className="flex-1 flex items-center gap-3">
                        ...
                        <form onSubmit={handleSearch} className="flex-1">
                            <div className="relative" data-tv-section="search-input">
                ...
                <>
                    ...
                    <div className="w-72 flex items-center justify-end gap-2 px-2">
                        ...
                        <button
                            ...
                            className="w-10 h-10 rounded-full flex items-center justify-center transition-all bg-red-500/10 text-red-400 hover:bg-red-500/20 hover:text-red-300"
                            title="Logout"
                        >
