...
class DownloadQueueManager {
    ...
    async failDownload(downloadId: string, reason: string) {
        ...
        if (info.attempts < this.MAX_RETRY_ATTEMPTS) {
            info.attempts++;
            console.log(
                `    Retrying download... (attempt ${info.attempts})`
            );
        ...
        try {
            ...
            if (info.albumId) {
                try {
                    ...
                    console.log(
                        ` Failed to remove album: ${error.message}`
                    );
                }
            }
            ...
            console.log(
                `   📢 Notifying ${this.unavailableCallbacks.length} callbacks about unavailable album`
            );
    ...
    private async clearFailedLidarrImports(): Promise<void> {
        try {
            ...
            console.log(
                ` Failed to check Lidarr queue: ${error.message}`
            );
        }
    }
    ...
    cleanupStaleDownloads(): number {
        ...
        for (const [downloadId, info] of this.activeDownloads) {
            ...
            if (age > this.STALE_TIMEOUT_MS) {
                console.log(
                    `🧹 Cleaning up stale download: "${info.albumTitle}" (${downloadId}) - age: ${Math.round(age / 60000)} minutes`
                );
                this.activeDownloads.delete(downloadId);
                cleanedCount++;
            }
        ...
        if (cleanedCount > 0) {
            console.log(`🧹 Cleaned up ${cleanedCount} stale download(s)`);
        }
    ...
    private async linkDownloadJob(downloadId: string, albumMbid: string) {
        console.log(
            `   🔗 Attempting to link download job for MBID: ${albumMbid}`
        );
        try {
            const { prisma } = await import("../utils/db");
            ...
            console.log(
                `   📋 Found ${existingJobs.length} job(s) with this MBID:`,
