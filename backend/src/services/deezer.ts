...
class DeezerService {
    ...
    /**
     * Normalize album title for searching
     * Strips common suffixes like "(Deluxe)", "(Remastered)", "[2CD]" etc.
     */
    private normalizeAlbumTitle(title: string): string {
        return title
            // Remove parenthetical content: (Deluxe), (Hi-Res Version), etc.
            .replace(/\s*\([^)]*\)\s*/g, ' ')
            // Remove bracketed content: [2CD], [Bonus Tracks], etc.
            .replace(/\s*\[[^\]]*\]\s*/g, ' ')
            // Remove common suffixes
            .replace(/\s*(deluxe|remastered|remaster|edition|version|expanded|bonus|explicit)\s*/gi, ' ')
    ...
    async getAlbumCover(
        ...
        // Try with original title first, then normalized title
        const titlesToTry = [albumTitle];
        const normalizedTitle = this.normalizeAlbumTitle(albumTitle);
        if (normalizedTitle !== albumTitle && normalizedTitle.length > 0) {
        ...
        for (const title of titlesToTry) {
            try {
                const response = await this.client.get("/search/album", {
                    params: this.getRequestParams({
                        q: `artist:"${artistName}" album:"${title}"`,
                        limit: 1,
                    }),
                });
