...
export class AudiobookCacheService {
    ...
    async syncAll(): Promise<SyncResult> {
        ...
        try {
            ...
            console.log(
                `📚 Found ${audiobooks.length} audiobooks in Audiobookshelf`
            );
    ...
    private async syncAudiobook(book: any): Promise<void> {
        ...
        // Series info
        const series =
            metadata.series?.[0]?.name || metadata.series?.name || null;
        const seriesSequence =
            metadata.series?.[0]?.sequence || metadata.series?.sequence || null;
    ...
    async getAudiobook(audiobookId: string): Promise<any> {
        ...
        if (
            ...
            console.log(
                `📚 Audiobook ${audiobookId} not cached or stale, syncing...`
            );
            try {
