...
class SoulseekService {
  ...
  async connect(): Promise<void> {
    ...
    this.client.on("peer-error", (error: Error) => {
      sessionLog("SOULSEEK", `Peer error: ${error.message}`, "DEBUG");
    });

    this.client.on("client-error", (error: unknown) => {
      const message = error instanceof Error ? error.message : String(error);
      sessionLog("SOULSEEK", `Client error: ${message}`, "ERROR");
    });

    this.client.on("listen-error", (error: Error) => {
      sessionLog("SOULSEEK", `Listen error: ${error.message}`, "ERROR");
    });
  ...
  private async ensureConnected(force: boolean = false): Promise<void> {
    ...
    if (
      ...
      throw new Error("Connection cooldown - please wait before retrying");
    }
  ...
  async searchTrack(
    ...
    const { searchWithStrategies } =
      await import("./soulseek-search-strategies");
  ...
  private cleanupFailedUsers(): void {
    ...
    for (const [
      username,
      cooldownUntil,
    ] of this.userConnectionCooldowns.entries()) {
      if (cooldownCleanup >= cooldownUntil) {
        cooldownsToDelete.push(username);
      }
    }
    for (const username of cooldownsToDelete) {
      this.userConnectionCooldowns.delete(username);
    }
  }
  ...
  private recordUserFailure(username: string): void {
    ...
    this.userConnectionCooldowns.set(
      username,
      now + this.USER_CONNECTION_COOLDOWN,
    );
  }
  ...
  private rankAllResults(
    ...
    const scored = availableResults.map((file) => {
      ...
      if (normalizedFilename.includes(normalizedArtist.replace(/\s/g, ""))) {
        score += 50;
      } else if (
        (artistFirstWord.length >= 3 &&
          normalizedFilename.includes(artistFirstWord)) ||
        (artistSecondWord && normalizedFilename.includes(artistSecondWord))
      ) {
        score += 35;
      }
      ...
      if (
        ...
        titleWords.length > 0 &&
        titleWords.some((w) => w.length > 4 && normalizedFilename.includes(w))
      ) {
        score += 25;
      }
      ...
      const quality = this.getQualityFromFilename(file.file, file.bitrate);
        ...
        (resolve) => {
          ...
          download.events.on("complete", () => {
            ...
            if (fs.existsSync(destPath)) {
              ...
              sessionLog("SOULSEEK", "File not found after download", "ERROR");
              resolve({
                success: false,
  ...
  async searchAndDownload(
    ...
    const searchResult = await this.searchTrack(
      artistName,
      trackTitle,
      albumName,
    );
  ...
  async searchAndDownloadBatch(
    ...
    const searchPromises = tracks.map((track) =>
      searchQueue.add(() =>
        this.searchTrack(track.artist, track.title, track.album).then(
          (result) => ({
            track,
            result,
          }),
