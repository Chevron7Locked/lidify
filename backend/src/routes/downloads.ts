...
router.post("/", async (req, res) => {
    try {
        ...
        console.log(
            `📥 Triggering Lidarr download: ${type} "${subject}" → ${rootFolderPath}`
        );
...
async function processArtistDownload(
    ...
    try {
        ...
        console.log(`   Found ${releaseGroups.length} albums/EPs from MusicBrainz`);
        ...
        for (const rg of releaseGroups) {
            ...
            if (existingJob) {
                console.log(`   Skipping "${albumTitle}" - already in download queue`);
                continue;
            }
            ...
            console.log(`   📀 Created job for: ${albumSubject}`);
...
router.get("/", async (req, res) => {
    try {
        const userId = req.user!.id;
        const { status, limit = "50", includeDiscovery = "false" } = req.query;
